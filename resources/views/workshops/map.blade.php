    ...
    <div class="col-lg-4">
        <div class="card mb-3 text-dark">
            <div class="card-header bg-white py-3">
                <h6 class="mb-0 fw-semibold text-dark"><i class="bi bi-funnel me-2 text-primary"></i>Filter Workshops</h6>
            </div>
            <div class="card-body">
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label class="form-label small text-muted">Filter by Service Category</label>
                ...
                <div class="mb-1">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="open-now-filter">
                        <label class="form-check-label small text-dark" for="open-now-filter">Show Open Now Only</label>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card text-dark">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h6 class="mb-0 fw-semibold text-dark"><i class="bi bi-shop me-2 text-success"></i>Nearby Centers</h6>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMapAndList(workshops) {
        ...
        workshops.forEach(w => {
            ...
            const popupContent = `
                <div style="min-width: 200px;" class="text-dark">
                    <h6 class="fw-bold mb-1" style="color: #1a56db;">
                        ${w.name}
                        ${w.is_verified ? '<i class="bi bi-patch-check-fill text-primary" title="Verified Workshop"></i>' : ''}
                    </h6>
                    <div class="small text-muted mb-1"><i class="bi bi-geo-alt me-1"></i>${w.address}, ${w.city}</div>
                    <div class="small text-muted mb-1"><i class="bi bi-telephone me-1"></i>${w.phone}</div>
                    <div class="small mb-1">
                        ${renderStars(w.rating_avg)} <span class="text-muted">(${w.total_reviews})</span>
                    </div>
                    <div class="small mb-2">
                        ${w.open_now ? '<span class="badge bg-success">Open Now</span>' : '<span class="badge bg-danger">Closed</span>'}
                    </div>
            ...
            card.className = 'card mb-2 p-3 workshop-card-item border text-dark';
            card.innerHTML = `
                ...
                <div class="small text-muted mb-1">${w.address}</div>
                <div class="small mb-1">${renderStars(w.rating_avg)} (${w.total_reviews})</div>
                <div class="small mb-2">
                    ${w.open_now ? '<span class="badge bg-success-subtle text-success border border-success">Open Now</span>' : '<span class="badge bg-danger-subtle text-danger border border-danger">Closed</span>'}
                </div>
    ...
    function applyFilters() {
        ...
        const filtered = allWorkshops.filter(w => {
            ...
            const matchesCat = cat === '' || w.categories.includes(cat);
            const matchesOpen = !openNowOnly || w.open_now;

            return matchesSearch && matchesCat && matchesOpen;
        });
